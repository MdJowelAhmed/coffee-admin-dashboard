...
## 11. Dashboard Mock Data (Current UI Match)

- `src/pages/Dashboard/dashboardData.ts` এ `carBookingsData` আর ব্যবহার করা হয় না।
- Dashboard table এখন `recentOrdersData` ব্যবহার করে এবং `Recent Order` UI এর সাথে aligned।
- `recentOrdersData` item shape:
  - `sl`, `itemImage`, `title`, `orderId`
  - `date`, `time`, `customer`, `itemCount`, `amount`, `status`
- `src/pages/Dashboard/RecentActivityCard.tsx` এর table headers:
  - `SL`, `Items`, `Title`, `Date & Time`, `Customer`, `Item Number`, `Amount`, `status`
- Status value supported: `Completed`, `Processing`, `Cancelled`
